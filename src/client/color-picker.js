(function () {
  ...
  const palette = families.flatMap((family) => family.colors);
      ...
      <div class="color-grid">
        ${families.map((family) => `
          <section class="color-family">
            <div class="color-family-title">${family.name}</div>
            <div class="color-family-swatches">
              ${family.colors.map((color) => `
                <button type="button" class="color-swatch ${color.toLowerCase() === current ? "is-selected" : ""}" data-color="${color}" style="--swatch:${color}" title="${family.name}: ${color}"></button>
              `).join("")}
            </div>
          </section>
        `).join("")}
  ...
  window.RythmColors = { palette, renderPicker };
}());
